...
class NoGroupingStrategy implements GroupingStrategy {

  @Override
  public List<AssetLibraryNode> getGroupedAssets(List<Asset> assets) {
    return assets.parallelStream().map(AssetNode::new).collect(Collectors.toList());
  }

  @Override
  public void insert(Asset asset, AssetGroupNode root) {
    root.add(new AssetNode(asset));
  }

  @Override
  public void remove(Asset asset, AssetGroupNode root) {
    root.remove(root.getChildFor(asset));
  }
}
